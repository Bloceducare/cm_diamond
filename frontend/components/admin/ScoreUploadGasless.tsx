...
import useUploadScoreGasless from "@/hooks/adminHooks/useUploadScoreGasless";
import sample from "../../public/admin/scoresample.png";
import Image from "next/image";

const ScoreUploadGasless = ({ apiKey, secretKey }: any) => {
  const [csvData, setCsvData] = useState([]);
  const [testId, setTestId] = useState<string | number | bigint | any>();
  ...
  const { uploadStudentsScore, isWriting } = useUploadScoreGasless(
    testId,
    csvData,
    apiKey,
    secretKey,
  ...
  const handleDataUpload = async (e: any) => {
    ...
    if (!testId)
      return toast.error("Please enter test Id", { position: "top-right" });
    if (csvData.length === 0)
      ...
      <main className="w-full flex flex-col gap-7">
        ...
        <div className="w-full px-20 flex flex-col gap-4 items-center">
          <div className="w-full flex flex-col">
            <label htmlFor="testId" className="text-color3 font-medium ml-1">
              Test Id
            </label>
            <input
              type="number"
...
export default ScoreUploadGasless;
